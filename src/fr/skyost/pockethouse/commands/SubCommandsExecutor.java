	...
	public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
		try {
			...
			if(commandInterface.mustBePlayer() && !(sender instanceof Player)) {
				PocketHouseAPI.log(ChatColor.RED, PocketHouseAPI.getPlugin().messages.messageNoConsole);
				return true;
			}
			final String permission = commandInterface.getPermission();
			if(permission != null && !sender.hasPermission(permission)) {
				PocketHouseAPI.log(ChatColor.RED, PocketHouseAPI.getPlugin().messages.messageNoPermission, sender);
				return true;
			}
